...
function fPaginatorHelper(options){
	...
	var paginatorClass = options.class || '';
	var self = this;
	var result = '';
	var i;
	...
	var currentPage = '<li class="current">' +
	...
	function pageLink(i){
		return '<li aria-label="Page ' + (transform ? transform(i) : i) + '"><a href="' + link(i) + '">' +
	...
	if (prevNext){
		// Not Disabled
		if (current > 1) {
			result += '<li class="pagination-previous"><a aria-label="Previous page" rel="prev" ' +
				'href="' + link(current - 1) + '">' + prevText + '</a></li>';
		} else {
		// Disabled
			result += '<li class="pagination-previous disabled">' + prevText + '</li>';
		}
	}
	...
	if (options.show_all){
		...
		var spaceHtml = '<li class="ellipsis" aria-hidden="true"></li>';
	...
	if (prevNext){
		if (current < total) {
			result += '<li class="pagination-next"><a aria-label="Next page" rel="next" ' +
				'href="' + link(current + 1) + '">' + nextText + '</a></li>';
		} else {
			result += '<li class="pagination-next disabled">' + nextText + '</li>';
		}
	}

	return '<ul class="pagination ' + paginatorClass + '" role="navigation" aria-label="Pagination">' + result + '</ul>';
}

hexo.extend.helper.register('f_paginator', fPaginatorHelper);